...
export function Sidebar({ prompts, filter, setFilter, accent, accentSoft, onClose, mobileOpen, accountProps, section, setSection }) {
  ...
  const item = (key, active, color, glyph, label, count, onClick) => (
    <div key={key} onClick={onClick} className="stash-row" style={{
  ...
  const goLibrary = (filterFn) => () => { setSection("library"); filterFn(); };
  ...
  return (
    <aside style={{
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "16px 12px", display: "flex", flexDirection: "column", gap: 18 }}>

        {/* Explore */}
        <div>
          {item("explore", section === "explore", "#0ea5e9", globeIcon, "Explore", null,
            () => setSection("explore"))}
        </div>
        ...
        <div>
          <div style={{ fontSize: 11, letterSpacing: 0.7, color: "var(--text-faint)", textTransform: "uppercase", fontWeight: 600, padding: "4px 10px 6px" }}>Models</div>
          {Object.entries(AI_META).map(([k, m]) =>
            item(k, section === "library" && filter.ai === k, m.color, m.glyph, m.name, counts[k] || 0,
              goLibrary(() => setFilter({ ...filter, scope: "all", ai: filter.ai === k ? null : k })))
          )}
        </div>
        ...
        {prompts.length > 0 && (
          <div style={{ padding: 10, borderRadius: 12, background: "var(--surface)", border: "1px solid var(--border)" }}>
            <div style={{ fontSize: 12.5, color: "var(--text-2)", fontWeight: 600, marginBottom: 2 }}>
              {prompts.reduce((a, p) => a + p.uses, 0)} runs this month
            </div>
            <div style={{ fontSize: 11.5, color: "var(--text-muted-2)" }}>Top: <i style={{ fontFamily: "'Instrument Serif', serif", fontStyle: "italic", color: "var(--text)", fontWeight: 500 }}>{prompts.slice().sort((a, b) => b.uses - a.uses)[0].title}</i></div>
          </div>
        )}
      </div>
